...
%preallocazione matrice per i risultati
...
for i = 1:num_rows
    ...
    for j = relative_period + 1:num_cols
        % Estrae i rendimenti per il periodo
...
% primi 3 asset per ciascun periodo
%
m_delta1 = m_delta(:, 5:end);  

% rendimenti dei primi 3 asset ordinati per ciascun mese (colonna)
...
% rendimenti medi dei primi 3 asset selezionati per ciascun mese
...
% Rimozione VFISX dal dataset per il calcolo del momentum relativo
m_delta_no_vfisx = m_delta;
m_delta_no_vfisx(4,:) = [];  % Rimuove la riga 4 (VFISX)% Rimuove la riga 4 (VFISX) 
...
for i = 1:num_rows
    for j = relative_period + 1:num_cols
        ...
        relmom(i, j-relative_period) = last_cumulative_return2; 
    end
end
...
wV = 0.5; % Peso della volatilità cash_proxy_index = 4; % VFISX come proxy del risk-free
num_days = size(d_delta, 2);
N=3;

cash_proxy_d= d_delta(4,:); 
d_delta(4,:) = []; % Rimuove la riga 4 (VFISX) 
...
for i = 1:num_assets
    ...
    % Prende solo i valori di volatilità corrispondenti alle date mensili (fine del mese)
...
for k = 1:(num_cols-relative_period)
    ...
    for asset_idx = 1:N
        ...
        if relmom(asset, k) > absolute_threshold
            ...
            RAV_returns(asset_idx, k) = cash_proxy_m(k);  % Investe nel (VFISX)
        end
    end
end
...
for t = relative_period + 1:num_cols
    ...
    if end_idx <= size(d_delta, 2)
        % Estrazioen rendimenti giornalieri per il periodo di lookback
        period_returns = d_delta(:, start_idx:end_idx);  
        
        % corr sui rendimenti giornalieri
        corr_mat = corrcoef(period_returns');

        % Calcolo della correlazione media per ogni asset rispetto agli altri (escludendo la diagonale)
