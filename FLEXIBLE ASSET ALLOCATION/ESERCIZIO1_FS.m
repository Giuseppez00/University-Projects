...
m_delta_no_vfisx(4,:) = [];  % Rimuovi la riga 4 (VFISX) %%%%%%%%%%  !!!!!!! %%%%%%%%%% devo ricordarmi di non rannare più volte uqesta linea , altrimenti mi riduce ogni volta la dimensione del dataset e il codice si interrompe.
...
for k = 1:size(m_delta2, 2)
    % Seleziona i rendimenti dei primi 3 asset (righe in idx_rel_mom2) per ciascuna colonna k
    RAselected_assets = idx_rel_mom2(1:3, k);  % Primi 3 asset selezionati per mese k
    
    for asset_idx = 1:3  % Itera sui 3 asset selezionati
        % Momentum relativo è già calcolato nella matrice 'relmom'
        last_cumulative_return = relmom(RAselected_assets(asset_idx), k); 
        
        % Se il momentum assoluto è > 1, prendi l'asset selezionato, altrimenti --> VFISX
...
d_delta(4,:) = [];% Rimuovi la riga 4 (VFISX) %%%%%%%%%%  !!!!!!! %%%%%%%%%% devo ricordarmi di non rannare più volte uqesta linea , altrimenti mi riduce ogni volta la dimensione del dataset e il codice si interrompe.
...
for t = relative_period + 1:num_cols
    ...
    if end_idx <= size(d_delta, 2)
        % Estrai i rendimenti giornalieri per il periodo di lookback
